...
def train(head_model, dataset, use_dataset, num_epochs, batch_size, learning_rate):
    ...
    if use_dataset == -1:    
        train, valid = model_selection.train_test_split(train, stratify=train['sentiment'], test_size=0.2, random_state=2020)
    else:
        train, _ = model_selection.train_test_split(train, stratify=train['sentiment'], train_size=use_dataset, random_state=2020)
        train, valid = model_selection.train_test_split(train, stratify=train['sentiment'], test_size=0.1, random_state=2020)
    ...
    embedding_path = 'fastText/embedding/cc.en.300.vec'
    print("\nLoad Embedding ...\n" + '==================\n')
