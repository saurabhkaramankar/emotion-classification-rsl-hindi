...
def load_data(train, valid, test, num_words, maxlen):
    ...
    tokenizer.fit_on_texts(list(train['review'].astype(str).values) + list(valid['review'].astype(str).values) + list(test['review'].astype(str).values))
    
    train_tokenized = tokenizer.texts_to_sequences(train['review'].astype(str))
    valid_tokenized = tokenizer.texts_to_sequences(valid['review'].astype(str))
    test_tokenized = tokenizer.texts_to_sequences(test['review'].astype(str))
